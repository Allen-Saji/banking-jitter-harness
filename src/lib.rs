...
mod tests {
    ...
    fn measure_banking_stage_slot_timing() {
        ...
        // One transfer per iteration; time send -> committed entry. For the
        // cold iteration (0) and a steady iteration we additionally snapshot
        // every worker thread's schedstat and the process fault/ctxt counters
        // so the spike can be attributed to a thread instead of guessed.
        ...
        for i in 0..N_ITERATIONS {
            ...
            let before_sched = probe.then(|| {
                thread_names().keys().map(|&t| (t, tid_schedstat(t))).collect::<BTreeMap<_, _>>()
            });
            let before_faults = probe.then(proc_faults);
            let before_ctxt = probe.then(proc_ctxt_switches);
            ...
            if probe {
                ...
                let before = before_sched.unwrap();
                // per-worker (cpu_ns, wait_ns) delta over the iteration window.
                // Only Agave threads are the system under observation; the
                // harness driver thread spins (yield_now) while waiting and
                // would otherwise dominate the table with spin-wait CPU.
                let mut rows: Vec<(String, u64, u64)> = names
                    .iter()
                    .filter(|(_, name)| name.starts_with("sol") || *name == "BankingMgr")
                    .filter_map(|(tid, name)| {
                        let (c1, w1) = tid_schedstat(*tid);
                        let (c0, w0) = before.get(tid).copied().unwrap_or((c1, w1));
                        let (dc, dw) = (c1.saturating_sub(c0), w1.saturating_sub(w0));
                        (dc + dw > 0).then(|| (name.clone(), dc, dw))
                    })
                    .collect();
                rows.sort_by_key(|r| std::cmp::Reverse(r.1));
                let (mnf, mjf) = proc_faults();
                let (bmnf, bmjf) = before_faults.unwrap();
                let (vc, nvc) = proc_ctxt_switches();
                let (bvc, bnvc) = before_ctxt.unwrap();
                let attr = (
                    elapsed_us,
                    rows,
                    (mnf - bmnf, mjf - bmjf),
        ...
        let print_attr = |label: &str, attr: &(u64, Vec<(String, u64, u64)>, (u64, u64), (u64, u64))| {
            let (wall_us, rows, (minf, majf), (vc, nvc)) = attr;
            println!("\n  [{label}] window {wall_us} us");
            println!("    per-thread (cpu_us, runqueue_wait_us), top 6:");
            for (name, dc, dw) in rows.iter().take(6) {
                println!("      {name:<16} cpu {:>8} us   wait {:>8} us", dc / 1000, dw / 1000);
            }
            println!("    page faults (process): minor {minf}  major {majf}");
            println!("    ctxt switches (driver thread): voluntary {vc}  involuntary {nvc}");
        };
        ...
        println!("\n  reading: the thread with the most cpu_us in the cold window did the");
        println!("  cold-start work; high minor-faults there means demand-paged code (lazy");
        println!("  loading); high involuntary ctxt-switches means the OS preempted it.");
